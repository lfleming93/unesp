...
/* shared variable - pointer to the shared hash table */
hash_table_t *ht;
...
void intset_init(long int size, unsigned short *seed)
{
  int i;
  for (i=0; i<size; i++)
    ht_add(ht, (int)(erand48(seed)*size*2));
}
...
void *intset_exercise(void *arg)
{
  ...
  int update_chance;  /* change (0-100) that we will update the table */
  ...
  while (parms->operations--)
  {
    ...
    if (update_chance < parms->upd_rate) {
      ...
      parms->num_found += ht_contain(ht, value);
      parms->num_search++;
    }
  }
...
int main(int argc, char *argv[])
{
  ...
  /* print the initial hash table size */
  fprintf(stdout, "=============== Initial state ===============\n");
  fprintf(stdout, "hash table size = %zu\n", ht_size(ht));
  fprintf(stdout, "=============================================\n");
